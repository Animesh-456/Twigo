...
if($_POST){
    // if(!empty($_POST["name"]) && !empty($_POST["address"]) && !empty($_POST["email"]) && !empty($_POST["contact"]) && !empty($_POST["city"]) && !empty($_POST["liscno"]) && !empty($_POST["dob"]) && !empty($_POST["gender"]) && !empty($_POST["psw"]){
        ...
        $psw = $_POST["psw"];
        $security = $_POST["sq"];
        $adhar = $_POST["Aadhar ID"];
        $dob= $_POST["dob"];

        $sql = "INSERT INTO customer(D_name, D_email, D_address, D_contact, D_city, D_lisence, D_dob, D_gender, 
        D_password, D_security, D_adhar) values('".$name."','".$email."', '".$address."', '".$contact."', 
